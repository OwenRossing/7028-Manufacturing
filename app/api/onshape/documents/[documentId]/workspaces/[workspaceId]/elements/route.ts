...
export async function GET(
  request: NextRequest,
  context: { params: Promise<{ documentId: string; workspaceId: string }> }
) {
  ...
  try {
    const items = await client.listElements(parsed.data.documentId, parsed.data.workspaceId);
    return NextResponse.json({ items: items.filter((item) => item.elementType.toUpperCase() === "ASSEMBLY") });
  } catch (error) {
    return jsonError(
      error instanceof Error ? `Onshape element lookup failed: ${error.message}` : "Onshape element lookup failed.",
      400
    );
  }
}
